...
const videoWidth = isMobile() ? 600*0.5 : 720,
      ...
      stats = new Stats(),
      minimumPartConfidence = 0.5,
      rightShoulderKeypointIndex = 6;

let calibrationExecutedForFirstPose = false,
    ...
    poses.forEach(({score, keypoints}) => {
      if (score >= minPoseConfidence) {
        ...
        if (guiState.output.showBoundingBox) {
          ...
          calibrationPoses(calibrationExecutedForFirstPose, keypoints).catch(error => null);
      ...
      if(calibrationContinue) {
          ...
          saveCalibratedPoints('rightLegLunge', lastPose.keypoints);
          calibrationContinue = false;
...
function calibrationPoses(executed, keypoints)
{
    if(!executed) {
        calibrationExecutedForFirstPose = true;
        ...
        return new Promise((resolve) => {
            ...
            saveCalibratedPoints('leftLegLunge', keypoints);

            console.log('leftLegLunge keypoints', keypoints);
            ...
            addImageOfPose('./images/lunge.jpg');
            ...
            new Promise((resolve) => {
                ...
                let downloadTimer = setInterval(() => {
                    timeleft--;
                    document.getElementById("countdowntimer").textContent = "The third pose calibration will end in " + timeleft + " seconds. Please hold still.";
                    if (timeleft <= 0) {
                        clearInterval(downloadTimer);
                        resolve()
                    }
      ...
      return new Promise((resolve, reject) => {
          reject('calibration lunge pose rejected')
      });
    }
}
...
function countCycles(keypoints) {
    if(calibrationPoints.size >= 3) {
        ...
        let leftLegLungeKeypoints = calibrationPoints.get('leftLegLunge'),
            isleftLegLunge = isAnyKeypointCloseToCalibrated(keypoints, leftLegLungeKeypoints),
        ...
        if (isStanding) {
            cycles.set('standing')
        } else if(isleftLegLunge ) {
            cycles.set('leftLegLunge')
        }
...
function isAnyKeypointCloseToCalibrated(keypoints, calibratedKeypoints) {
    for(let i = rightShoulderKeypointIndex; i >= 0 ; i-- ) {
        let keypoint = keypoints[i],
            calibrateKeypoint = calibratedKeypoints[i];

        if (minimumPartConfidence < keypoint.score) {
            return isKeypointCloseToCalibrated(
                keypoint.position.x,
                calibrateKeypoint.position.x,
                keypoint.position.y,
...
function isKeypointCloseToCalibrated(x1, xCalibrated, y1, yCalibrated) {
